...
public interface CommentMapper {
    @Insert("INSERT INTO note_comment (comment_text, author_id, note_id, revision_id) " +
            "VALUES ( #{comment.commentBody}, #{comment.author.id}, #{comment.section.id}, #{comment.noteRevision.revisionId})")
    @Options(useGeneratedKeys = true, keyProperty = "comment.id", keyColumn = "id")
    Integer saveComment(@Param("comment") Comment comment);

    @Select("SELECT id, comment_text as commentBody, author_id, note_id, revision_id, " +
    ...
    @Select("SELECT id, comment_text as commentBody, author_id, note_id, revision_id, " +
            "comment_creation_time as creationTime FROM note WHERE note_id = #{noteId} ")
    @Results({
            @Result(property = "author", column = "author_id", javaType = User.class,
                    one = @One(select = "net.thumbtack.school.notes.mappers.UserMapper.getUserById", fetchType = FetchType.LAZY)),
            @Result(property = "note", column = "note_id", javaType = Note.class,
    ...
    @Update("UPDATE note_comment SET comment_text = #{newCommentBody} WHERE id = #{commentId}")
